...
void oled_render(void) {
    ...
    uint8_t* send_buffer = oled_buffer;
    uint8_t oled_glitches[] = {-2, 0, 2, 0, 0, 0};

    if (update_start < 2) {
        static uint8_t glitch_buffer[OLED_MATRIX_SIZE];
        memset(glitch_buffer, OLED_BLOCK_SIZE * update_start, sizeof(OLED_BLOCK_SIZE));
        uint8_t pos = OLED_BLOCK_SIZE * update_start;
        int8_t rota, rotb;

        for (uint8_t i = 0; i < OLED_BLOCK_SIZE; i++) {
            rota = (i + oled_glitches[update_start << 1]) % OLED_BLOCK_SIZE;
            rotb = (i + oled_glitches[(update_start << 1) + 1]) % OLED_BLOCK_SIZE;

            glitch_buffer[pos + i] = (oled_buffer[pos + rota] & 0b11110000) | (oled_buffer[pos + rotb] & 0b00001111);
        }

        send_buffer = glitch_buffer;
    }


    if (!HAS_FLAGS(oled_rotation, OLED_ROTATION_90)) {
        // Send render data chunk as is
        if (I2C_WRITE_REG(I2C_DATA, &send_buffer[OLED_BLOCK_SIZE * update_start], OLED_BLOCK_SIZE) != I2C_STATUS_SUCCESS) {
        ...
        for (uint8_t i = 0; i < sizeof(source_map); ++i) {
            rotate_90(&send_buffer[OLED_BLOCK_SIZE * update_start + source_map[i]], &temp_buffer[target_map[i]]);
        }
